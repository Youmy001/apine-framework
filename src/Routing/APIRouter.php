...
final class APIRouter implements RouterInterface
{
    ...
    public function route($request)
    {
        try {
            ...
            if (self::check_route($request)) {
                $route = new Route($controller, strtolower(Request::getRequestType()), $args);
            }
    ...
    private function checkRoute($route)
    {
        ...
        try {
            ...
            if (class_exists('Apine\\Controllers\\User\\' . $maj_controller) && in_array('Apine\\MVC\\APIActionsInterface',
                    class_implements('Apine\\Controllers\\User\\' . $maj_controller))
            ) {
                $return = 'Apine\\Controllers\\User\\' . $maj_controller;
            } else {
                if (class_exists('Apine\\Controllers\\System\\' . $maj_controller) && in_array('Apine\\MVC\\APIActionsInterface',
                        class_implements('Apine\\Controllers\\System\\' . $maj_controller))
                ) {
    ...
    {
        $controller_name = self::checkRoute("/$controller/$method");
