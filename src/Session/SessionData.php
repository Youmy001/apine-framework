...
final class SessionData extends EntityModel
{
     ...
     * @return boolean
     */
    public function isValid($delay = 7200)
    {
        return (strtotime($this->getLastAccess()) > time() - $delay);
    }
     ...
     * @return mixed
     */
    public function getVariable($a_name)
    {
        if ($this->data === null) {
            $this->load();
        }
     ...
     * @param mixed  $a_value
     */
    public function setVariable($a_name, $a_value)
    {
        if ($this->data === null) {
            $this->load();
        }
     ...
     * @param string $a_name
     */
    public function removeVariable($a_name)
    {
        if ($this->data === null) {
            $this->load();
        }
