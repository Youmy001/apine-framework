...
class ApineImageFactory extends ApineFactory implements ApineFactoryInterface{
	...
	public static function is_id_exist($image_id){
		...
		$id_sql = self::_get_connection()->select("SELECT ID FROM images WHERE ID=$image_id");
		if($id_sql){
			return true;
		}
		return false;
	...
	public static function is_access_id_exist($image_id){
		...
		$id_sql = self::_get_connection()->select("SELECT ID FROM images WHERE access_id='$image_id'");
		if($id_sql){
			return true;
		}
		return false;
	...
	public static function create_all(){
		...
		$request = self::_get_connection()->select('SELECT ID from `images` ORDER BY `ID`');
		$liste = new Liste();
	...
	public static function create_all_folder($a_folder){
		...
		$request = self::_get_connection()->select("SELECT ID from `images` where folder=$a_folder ORDER BY `ID`");
		$liste = new Liste();
		if($request != null && count($request) > 0){
			foreach($request as $item){
				$liste->add_item(new ApineImage($item['ID']));
	...
	public static function create_by_id($a_id){
		...
		$sql_id = self::_get_connection()->prepare('SELECT ID FROM `images` WHERE ID=?');
	...
	public static function create_by_access_id($access_id){
		...
		$sql_id = self::_get_connection()->prepare('SELECT ID FROM `images` WHERE access_id=?');
		$ar_sql = self::_get_connection()->execute(array(
						$access_id
		), $sql_id);
		if($ar_sql){
	...
	/**
	 * Fetch all images owned by a user
	...
	public static function create_by_user($a_user_id){
		...
		$request = self::_get_connection()->select("SELECT ID from `images` where user_id=$a_user_id");
		$liste = new Liste();
		if($request != null && count($request) > 0){
			foreach($request as $item){
				$liste->add_item(new ApineImage($item['ID']));
	...
	/**
	 * Fetch all images owned by a user in a folder
	 * @param integer $a_user_id
	 *        User identifier
	 * @param string $a_folder
	 *        Folder name
	...
	public static function create_by_user_folder($a_user_id, $a_folder){
		...
		$request = self::_get_connection()->select("SELECT ID from `images` where user_id=$a_user_id AND folder=$a_folder");
		$liste = new Liste();
		if($request != null && count($request) > 0){
			foreach($request as $item){
				$liste->add_item(new ApineImage($item['ID']));
	...
	/**
	 * Fetch all user's public images
	...
	public static function create_by_user_public($a_user_id){
		...
		$request = self::_get_connection()->select("SELECT ID from `images` where user_id=$a_user_id OR privacy=2");
		$liste = new Liste();
		if($request != null && count($request) > 0){
			foreach($request as $item){
				$liste->add_item(new ApineImage($item['ID']));
	...
	/**
	 * Fetch user's public images in a folder
	 * @param integer $a_user_id
	 *        User identifier
	 * @param string $a_folder
	 *        Folder name
	...
	public static function create_by_user_folder_public($a_user_id, $a_folder){
		...
		$request_id = self::_get_connection()->prepare("SELECT ID from `images` where ( user_id=? AND folder=? ) OR ( privacy=2 AND folder=? )");
	...
	/**
	 * Fetch all images by privacy level
	 * @param integer $privacy Privacy level
	 * @return Liste
	 */
	public static function create_by_privacy($privacy){

		$request = self::_get_connection()->select("SELECT ID from `images` where privacy=$privacy");
		$liste = new Liste();
		if($request != null && count($request) > 0){
			foreach($request as $item){
				$liste->add_item(new ApineImage($item['ID']));
