 ...
 *     database. Optional.
 */
abstract class EntityModel implements
    EntityInterface,
    Serializable,
    JsonSerializable
{
    ...
    final public function __call(string $a_name, array $a_arguments)
    {
        ...
        try {
            switch ($action) {
                ...
                case 'get':
                    if ((property_exists($this, $property) && array_key_exists($property,
                    ...
                    if ((property_exists($this, $property) && array_key_exists($property,
                        ...
                        if (property_exists($this, $property)) {
                            ...
                            if ($property !== 'id' && $property !== $this->key_field) {    // Prevent to execute the modification to the database
                                ...
                                if (is_array($this->field_mapping) && (false !== ($field = array_search($property,
                                            $this->field_mapping)))
                                ) {
     ...
     */
    final private function dataLoading()
    {
     ...
     */
    final private function propertyLoading()
    {
        if (!$this->values_loaded) {
            $this->dataLoading();
            $properties = get_class_vars(get_class($this)); // List of accessible properties
            ...
            foreach ($this->initial_values as $name => $value) {
                ...
                if (property_exists($this, $name) && array_key_exists($name,
                        $properties)
                ) {    // Should try to assign the value only
    ...
    final private function rollback()
    {
        ...
        $this->propertyLoading(); // Reload the properties
    ...
    final protected function get(string $a_field)
    {
        ...
        $values_array = array_merge($this->initial_values,
            $this->edited_values);    // Concatenate the edited and initial values
        // to get a list of every current values
        
        if (isset($values_array[$a_field])) {
            $value = $values_array[$a_field];
            
            $adjust = Application::getInstance()->getConfig()->entity->adjust_timestamp;
     ...
     * @return array
     */
    final protected function getAll()
    {
        if (empty($this->initial_values)) {
            $this->dataLoading();
        }
        
        return array_merge($this->initial_values,
            $this->edited_values); // Concatenate the initial state with the updated state.
    }
     ...
     * @param Database       $database Instance of a database to commit changes to. If empty, will try to connect to
    ...
    private function array_serialization($object, $all)
    {
        ...
        foreach ($object as $array_name => $array_item) {
            if ($array_item instanceof EntityModel) {
                $master_array[$array_name] = $array_item->serialize(!$all);
            } else {
                if (is_array($array_item) || $array_item instanceof Traversable) {
