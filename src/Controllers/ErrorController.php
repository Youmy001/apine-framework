...
class ErrorController extends MVC\Controller
{
    ...
    {
        if (Core\Request::isApiCall() || Core\Request::isAjax()) {
        ...
        if ($a_exception !== null && !is_array($a_exception)) {
            ...
            if (Application\Application::getInstance()->isDebugMode()) {
                $view->set_param('trace', $a_exception->getTraceAsString());
            }
        }
        
        if ($this->isHttpCode($a_code)) {
     ...
     * @return boolean
     */
    public function methodForCode($a_code)
    {
     ...
     * @return boolean
     */
    private function isHttpCode($a_code)
    {
