	...
	<div class="col-md-4 col-md-offset-4">
		...
		<div class="panel panel-default">
			...
			<?php 	if ($this->_params['error_code'] == 200) :?>
			...
			<div class="panel-body">
				<form id="content" action="<?= ApineURLHelper::path("login/restore/".$this->_params['token']->get_token());?>"
					method="post">
					<div class="form-group">
						<label class="control-label"><?= ApineAppTranslator::translate('restore','username'); ?></label>
						<input class="form-control" type="text" name="user"
								placeholder="<?= ApineAppTranslator::translate('restore','username'); ?>" disabled="disabled" value="<?= $this->_params['token']->get_user()->get_username() ?>" />
					</div>
					<div class="form-group">
						<label class="control-label"><?= ApineAppTranslator::translate('restore','email'); ?></label>
						<input class="form-control" type="email" name="email"
								placeholder="<?= ApineAppTranslator::translate('restore','email'); ?>" disabled="disabled" value="<?= $this->_params['token']->get_user()->get_email_address() ?>" />
					</div>
					<div class="form-group">
					...
					<div class="checkout">
						...
						<input type="hidden" name="token" value="<?= $this->_params['token']->get_token(); ?>" />
