...
class SessionController extends MVC\Controller {
				...
				if ((isset($user) && isset($pwd)) && (!Session\SessionManager::is_logged_in())) {
					if (Session\SessionManager::login($user, $pwd, array("remember" => $permanent))) {
						if (isset($redirect) && $redirect != "") {
							$redirection = Routes::internal_redirect($redirect);
						} else {
							$redirection = Routes::internal_redirect("");
						}
	...
	public function logout () {
		...
		return Routes::internal_redirect('login');
	...
	public function register ($params) {
		...
		if (Core\Request::is_post() && !Session\SessionManager::is_logged_in()) {
			...
			try {
				...
				if ((isset($user) && isset($pwd) && isset($pwdconfirm) && isset($email)) && (!Session\SessionManager::is_logged_in())) {
			...
			$view->set_param('error_code',true);
			$view->set_param('error_message', $message);
		}
		...
		$view->set_title($title);
		$view->set_view('session/register');
		$view->set_response_code(200);
		return $view;
	...
	public function api () {
        ...
        $view = new MVC\TwigView();
		$view->set_response_code(200);
		$view->set_title('API Instructions');
		$view->set_view('session/api');
