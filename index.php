...
if (ApineAppConfig::get('runtime', 'mode') == 'development') {
	ini_set('display_errors', -1);
	error_reporting(E_ALL | E_STRICT);
} else if (ApineAppConfig::get('runtime', 'mode') == 'production') {
	ini_set('display_errors', 0);
	error_reporting(E_ERROR);
}

if (ApineAppConfig::get('runtime', 'use_composer') === "yes") {
	require_once('vendor/autoload.php');
}
...
try {
	...
	if (ApineAppConfig::get('runtime', 'mode') != 'development' && ApineAppConfig::get('runtime', 'mode') != 'production') {
		throw new ApineException('Invalid Execution Mode \"'.ApineAppConfig::get('runtime', 'mode').'"', 418);
	}
	
	// Verify is the protocol is allowed
	if (ApineRequest::is_https() && ApineAppConfig::get('runtime', 'use_https') == 'no') {
		internal_redirect(ApineRequest::get()['request'], APINE_PROTOCOL_HTTP);
	}
	
	if (ApineAppConfig::get('runtime', 'route_format') == 'json') {
	...
	} else if (is_null(ApineAppConfig::get('runtime', 'route_format'))) {
		if (file_exists('routes.xml')) {
			if (!file_exists('routes.json')) {
				file_put_contents('routes.json', json_encode(export_routes('routes.xml'), JSON_PRETTY_PRINT));
			}
			ApineAppConfig::set('runtime', 'route_format', 'json');
		} else if (!file_exists('routes.xml') && !file_exists('routes.json')) {
			throw new ApineException('Route File Not Found', 418);
		}
	} else if (ApineAppConfig::get('runtime', 'route_format') != 'xml') {
	...
	if (ApineSession::is_logged_in()) {
		if (ApineAppConfig::get('runtime', 'secure_session') == 'yes') {
			if (!ApineRequest::is_https() && ApineAppConfig::get('runtime', 'use_https') == 'yes') {
				internal_redirect(ApineRequest::get()['request'], APINE_PROTOCOL_HTTPS);
			} else if (ApineRequest::is_https() && ApineAppConfig::get('runtime', 'use_https') == 'no') {
				internal_redirect(ApineRequest::get()['request'], APINE_PROTOCOL_HTTP);
			}
		} else {
	...
	if (!ApineRequest::is_api_call()) {
		...
		if (ApineAppConfig::get('runtime', 'use_api') == 'yes') {
			$request = ApineRequest::get()['request'];
		} else {
