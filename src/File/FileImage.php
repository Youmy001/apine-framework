...
final class FileImage extends File
{
        ...
        if (!$this->readonly) {
            ...
            if (($crop_h != $this->height()) || ($crop_w != $this->width())) {
                ...
                if ($crop_h < $this->height()) {                        // If current heigth is bigger
                    ...
                    if ($crop_h > $this->height()) {                // If current height is smaller
                        $crop_h = $this->height();
    ...
    public function resize($n_width, $n_height)
    {
        if (!$this->readonly) {
            $new_image = imagecreatetruecolor($n_width, $n_height);
            imagecopyresized($new_image, $this->image, 0, 0, 0, 0, $n_width, $n_height, $this->width(),
                $this->height());
        ...
        if (!$this->readonly) {
            if (!is_null($a_image)) {
                if (($this->type() == "image/png") || ($this->type() == "image/PNG")) {
                    ...
                    if (($this->type() == "image/jpg") || ($this->type() == "image/jpeg") || ($this->type() == "image/JPG") || ($this->type() == "image/JPEG")) {
